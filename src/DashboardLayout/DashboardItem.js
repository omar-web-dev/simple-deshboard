                ...
                >
                    ...
                    >
                        ...
                        <h2
                            ...
                            className={`whitespace-pre duration-500 ${openDrawer ? "opacity-0 md:opacity-100 translate-x-10 md:translate-x-0 overflow-hidden" : "md:opacity-0 md:translate-x-28 overflow-hidden"} `}
                        >{name}</h2>
                        <h2
                ...
                >
                    ...
                    >
                        ...
                        <h2
                            ...
                            className={`whitespace-pre duration-500 ${openDrawer ? "opacity-0 md:opacity-100 translate-x-10 md:translate-x-0 overflow-hidden" : "md:opacity-0 md:translate-x-28 overflow-hidden"} `}
                        >{name}</h2>
                        {subMenu && <IoMdArrowDropdown className={`ml-auto text-lg mt-[2px] ${subItemShow === name && "-rotate-90"} text-white group-hover:text-yellow-200`} />}
                        <h2
                            className={`${!openDrawer ? "hidden md:block" : "block md:hidden"}
                     absolute left-48 bg-gray-200 font-sans whitespace-pre z-30
                   text-gray-900 rounded-md drop-shadow-md px-0 py-0 w-0 overflow-hidden group-hover:px-2 
                ...
                >
                    {subMenu.map((menu, i) => (
                        <NavLink
                            key={i}
                            to={menu.subLink}
                            className={({ isActive }) => isActive ? "font-semibold  flex items-center bg-green-600 gap-3 pl-2 pr-4 text-white py-1 my-2" : "font-semibold  flex items-center hover:bg-green-600 gap-3 pl-2 pr-4 hover:text-white py-1 my-2"}
                        >
